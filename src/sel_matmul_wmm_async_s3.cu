...
#pragma unroll
    ...
    // Load A matrix tiles from gmem into shared memory using async copy
#pragma unroll
    ...
    // Load B matrix tiles from gmem into shared memory using async copy
#pragma unroll
    for (size_t i = 0; i < B_smem_iters; ++i) {
        ...
        // Update the pointers for the next set of tiles
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        ...
        // Load B matrix tile into fragment
...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            ...
            // Load next A matrix tile into fragment
            wmma::load_matrix_sync(A_frag[reg_store_idx][i], A_tile_ptr, AB_SMEM_STRIDE);
        }
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            ...
            // Load next B matrix tile into fragment
            wmma::load_matrix_sync(B_frag[reg_store_idx][j], B_tile_ptr, AB_SMEM_STRIDE);
        }
